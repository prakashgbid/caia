/**
 * Ollama embedder for Librarian Phase-1.
 *
 ...
 *
 * Subscription mandate: this is the only LLM-calling code Librarian
 * adds, and it talks exclusively to a local Ollama daemon — zero
 * marginal cost, zero API-key billing.
 *
 * This module is a 1:1 port of `@chiefaia/mentor-retrieval/embed.ts`.
 * We re-implement here rather than depend on that package directly to
 * keep Librarian's package surface independent — Mentor and Librarian
 * are sibling building blocks, not parent + child.
 *
 * Hard requirements:
