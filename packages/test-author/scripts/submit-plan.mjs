 ...
 *
 * Mirrors `packages/per-story-tester/scripts/submit-plan.mjs` verbatim.
 */
...
async function main() {
  ...
  if (stub) {
    ...
    critic = {
      review: async () => ({
        ...
        reasoning:
          'Stub critic: package surface matches Stage 10 brief (agent + persistence + contract + api), state-machine transitions stay within the canonical FSM (ea-complete -> tests-authored | tests-authored -> tests-authoring-failed), reuses @caia/architect-kit + @chiefaia/claude-spawner + @chiefaia/playwright-config + @chiefaia/test-kit + @chiefaia/ticket-template, distinct from @caia/testing-architect (strategy) and @caia/test-reviewer (audit). Suggest operator run a live review before merge.',
        cited_adrs: [],
        cited_principles: [],
        cited_lessons: [],
  ...
  const outcome = await agent.submitPlan({
    ...
    callerAgentId: '@caia/test-author',
