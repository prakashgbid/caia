...
export function matchScopePattern(pattern: string, value: string): boolean {
  ...
  // The pattern is operator-supplied policy text, not user input.
  // Allow-list glob patterns (`*`-only) are pre-escaped above and
  // bounded by the registry's static catalogue.
  const re = new RegExp('^' + escaped.replace(/\*/g, '.*') + '$'); // nosemgrep: javascript.lang.security.audit.detect-non-literal-regexp.detect-non-literal-regexp
