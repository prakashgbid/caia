/**
 * Unified-diff parser. Pure function — no I/O.
 *
 * Handles the GitHub-style unified diff format produced by `git diff` /
 * `gh pr diff`. Specifically:
 *   - `diff --git a/<path> b/<path>` separator lines
 *   - `--- a/<path>` / `+++ b/<path>` file headers (or `/dev/null`)
 ...
 *
 * Binary-file diffs are skipped (`Binary files <a> and <b> differ`).
 */
...
export function parseDiff(diff: string): ParsedDiff {
  ...
  for (const line of lines) {
    ...
    if (inHunk && currentFile !== null && !isBinary) {
      // accept ` `, `+`, `-`, `\` (no-newline) prefixed body lines
 ...
 * (Datadog BewAIre lesson). Sub-hunks share `file` / `oldStart` / `newStart`
 * (the first sub-hunk gets the original starts; subsequent ones get adjusted
 * starts based on line counts of preceding sub-hunks).
 */
export function chunkHunk(hunk: DiffHunk, maxBytes: number): DiffHunk[] {
  ...
  let oldOffset = 0;
  let newOffset = 0;
  // Track the offset for the NEXT bucket — committed when we flush.
  let nextOldOffset = 0;
  let nextNewOffset = 0;
...
export interface DiffLine {
  ...
  kind: '+' | '-' | ' ';
  /** New-file line number, 1-based. -1 for removed/context lines that don't
   * advance new-file counter. */
  newLine: number;
