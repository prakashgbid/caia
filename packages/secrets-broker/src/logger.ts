 ...
 * broker's default bindings and an HTTP bus transport that POSTs warn/
 * error/fatal lines as `system.error` events back to the orchestrator's
 * /events endpoint.
 *
 * Wiring is opt-in via `BROKER_BUS_URL` so the broker stays self-contained
 * when run without a Conductor next to it.
 */
