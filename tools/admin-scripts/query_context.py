...
class ContextQuery:
    ...
    def get_latest_context(self) -> Dict[str, Any]:
        ...
        # Only match context files with timestamp format: context_YYYYMMDD_HHMMSS.json
        import re
        context_pattern = re.compile(r'^context_\d{8}_\d{6}\.json$')
        context_files = sorted([
            f for f in self.context_dir.glob("context_*.json")
            if context_pattern.match(f.name)
        ])
        if context_files:
    ...
    def get_context_history(self, days_back: int = 7) -> List[Dict[str, Any]]:
        ...
        for context_file in sorted(self.context_dir.glob("context_*.json")):
            if not context_pattern.match(context_file.name):
                continue
