...
class AutomatedProgressCollector:
    ...
    def auto_generate_accomplishments(self, commits):
        ...
        for commit in commits:
            # Parse timestamp - handle various git timestamp formats
            timestamp_str = commit["timestamp"]
            try:
                # Try direct parsing first
                commit_time = datetime.fromisoformat(timestamp_str)
            except ValueError:
                # Handle git format: "2025-08-17 22:46:28 -0400"
                if " " in timestamp_str:
                    ...
                    if len(parts) == 2:
                        ...
                        timestamp_str = f"{date_part.replace(' ', 'T')}{tz_part}"
                        commit_time = datetime.fromisoformat(timestamp_str)
                    else:
    ...
    def update_repo_progress(self, repo_path, hours_back=1):
        ...
        progress_file = os.path.join(repo_path, "PROGRESS", "2025-08", "daily", f"{datetime.now().strftime('%Y-%m-%d')}.json")
...
def main():
    ...
    if args.command == "update-repo":
        ...
        # Handle both repo paths and repo names
        repo_path = args.repo
        if not os.path.exists(repo_path):
            # Try as repo name in projects root
            repo_path = os.path.join(collector.projects_root, args.repo)
        
        if not os.path.exists(repo_path):
            print(f"Error: Repository path not found: {args.repo}")
            return
        
        new_accomplishments = collector.update_repo_progress(repo_path, args.hours)
        print(f"✅ Updated {os.path.basename(repo_path)}: {new_accomplishments} new accomplishments")
