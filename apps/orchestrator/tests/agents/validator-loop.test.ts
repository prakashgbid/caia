 ...
 *
 * Verifies the BA → Validator → re-invoke-BA cycle:
 *   - Stories that pass on first attempt → validation_status = 'passed'.
 *   - Stories that fail twice → validation_status = 'escalated' + blocker filed.
 *   - Loop advances the prompt to `validated` pipeline stage exactly once.
 *   - Loop is idempotent on already-passed stories.
 */
...
function buildBrokenTicket() {
  ...
  // Inject forbidden snippet — this fails detail-sufficiency, soft severity.
...
describe('runValidatorLoop — happy path', () => {
  it('passes a high-quality story on first attempt and advances the pipeline', async () => {
    ...
    const out = await runValidatorLoop(
      ...
      db,
      { judge: allGreenJudge, maxAttempts: 2 },
    );
    ...
    // Pipeline advanced exactly once.
    const stageRows = db
      .select()
...
describe('runValidatorLoop — multi-story prompt', () => {
  it('validates each eligible story independently and aggregates counts', async () => {
    ...
    // Pipeline advances once for the prompt regardless of per-story outcomes.
    ...
    // Pipeline-stage metadata captures the aggregate counts.
