...
describe('DASH-302 seedFeatures', () => {
  ...
  it('inserts all seed rows on a fresh db', async () => {
    ...
    // Every row carries the seed marker so the dedup logic on next boot
    // can find it without an extra column.
  ...
  it('resolves project slugs to project ids', async () => {
    await seedFeatures(db);
    // The seed list includes a `pokerzeno`-scoped feature; verify its
    // projectId resolves rather than landing as null.
...
describe('DASH-303 seedSuggestions', () => {
  ...
  it('does NOT touch user-created suggestions on re-run', async () => {
    ...
    // Simulate a real (non-seed) suggestion arriving via POST /suggestions
    ...
    // User row still present unchanged
