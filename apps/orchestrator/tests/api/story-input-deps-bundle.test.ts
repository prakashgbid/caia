...
describe('migration 0025 — inputDependencies on /stories/:id/bundle', () => {
  ...
  it('drops non-object entries from input_dependencies_json (defensive parsing)', async () => {
    ...
    db.insert(stories).values({
      ...
      kind: 'story',
      status: 'pending',
      // Mixed garbage: should drop the string and number, keep the object.
  ...
  it('returns [] when input_dependencies_json is invalid JSON', async () => {
    const db = getDb();
    const id = 'st_broken_json';
    // Use raw SQL because drizzle's typed builder won't let us write garbage.
    const better = require('better-sqlite3') as (path: string) => {
      prepare: (s: string) => { run: (...args: unknown[]) => unknown };
      close: () => void;
    };
    const raw = better(dbPath);
    raw.prepare(
      `INSERT INTO stories (id, title, kind, status, created_at, input_dependencies_json)
       VALUES (?, ?, 'story', 'pending', ?, ?)`,
    ).run(id, 'Broken', new Date().toISOString(), '{not json');
    raw.close();
  ...
  it('input deps are independent of taxonomy.blockedBy (different fields)', async () => {
    ...
    // inputDependencies surfaces the capability requirement
    expect(body.inputDependencies).toHaveLength(1);
    expect(body.inputDependencies[0]!.name).toBe('login flow');
    // dependencies (story-to-story) is empty because we didn't set dependsOnJson
