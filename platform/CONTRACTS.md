...
| KID-017 | **Envelope reconciliation:** `packages/contracts` `FactoryTaskEnvelope` is NOT a competing wire envelope — it is the `data` payload schema of `com.caia.run.requested.v1` CloudEvents messages (§3.2 remains the only wire envelope, per STOL-1034 "every event conforms to CloudEvents 1.0"). Mapping at the boundary: `taskId`→`id`, `sfId`→`caiafactoryid`, `correlationId`→`caiarunid`+`subject`, `emittedAt`→`time`, `payload`→`data.payload`. TS + Python SDKs must both apply this mapping. |
| KID-018 | **SDK languages:** Python `caia-factory-sdk` (§6) remains the canonical Wave-1 SDK and the SF-HELLO acceptance vehicle. `packages/factory-sdk-ts` is a sanctioned secondary client for TS factories/UI; it must emit the same CloudEvents envelope and pass the same OPA/provenance calls. |
| KID-019 | **Topic scheme amendment (supersedes §3.1 table):** shipped Wave-1 topics are per-event-type: `caia.factory.artifact.published`, `caia.factory.ci.result`, `caia.factory.commit.result`, `caia.factory.evidence.appended`, `caia.factory.functional.result`, `caia.factory.requires_rework`, `caia.governance.kill`. Naming rule: `caia.<domain>.<event>` — NO version suffix on topics; versioning lives in the CloudEvents `type` field (KID-011). Partition key remains `run_id`/`correlationId`. New topics require a PR to this file. |
| KID-020 | **MinIO bucket family (extends §5.1):** `caia-artifacts` (canonical outputs), `caia-evidence` (gate evidence), `caia-context` (compiled context), `caia-cache`, `caia-traces` — all versioning ON, all provenance-tracked through `caia.artifact_provenance`. caia-minio-init exit-127 is cosmetic (missing grep in mc image final check) — buckets verified created + versioned. |
